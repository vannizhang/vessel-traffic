...
import { getAISFileSize } from '../../services/getAISFileSize';
...
const HOST_SERVER_URL = 'https://esri-vessel-traffic.s3.us-east-1.amazonaws.com'
...
const DownloadScreen:React.FC<Props> = ({
    ...
    const fileName = useMemo(()=>{
        if(selectedENCFeature){
            return `${selectedENCFeature.attributes['File_']}`;
        }

        return `${getFilenameByActiveLayerTime(activeLayerTimeInfo)}.zip`
    }, [selectedENCFeature, activeLayerTimeInfo])

    const formattedFileSize = useMemo(()=>{
        const size = getAISFileSize(fileName as any)
        ...
        >
            ...
            >
                ...
                >
                    <div>
                        <a
                            href={`${HOST_SERVER_URL}/${fileName}`}
                            target='_blank'
                        >
                            {/* { DownloadIcon } */}
                            <calcite-icon icon="download" scale="l"></calcite-icon>
                        </a> 
                        <br/>
                        <span className='text-sm'>{formattedFileSize}</span>
                    </div>
                </div>

                <div
                    className='ml-4 mr-12 text-sm'
                >
